...
;	一、DOS 屏幕
;		80x25 彩色字符模式：
;			显示器可以显示25行，每行80个字符(Byte). 每个字符可以有2^8=256种属性(闪烁，背景色，高亮，前景色等组合信息).
...
;	>>>
;	屏幕行数	对应显示缓冲器
;	1 (80byte)	00h-9Fh   (1*80x2 = 160byte = A0h)
;	2			A0h-13Fh  (2*80x2 = 140h)
;   3			140h-1DFh
;	...			...			...
;	
;
...
;		蓝底白字：		00010111b = 17h
;		白底闪烁蓝字：	11110001b (PS: 闪烁功能需要在全屏DOS下才有效！DOSbox非全屏也有效果.)
;
...
;		(闪烁)白底蓝色的'Welcome to zhq3211.' -- 第15行，对应显示缓冲区的 B800:0BE0h~B800:0C7Fh
;///////////////////////////////////////////////////////////////////////////////
assume cs:code
...
S1:	pop ax
	...
	mov ax, 02h			;字符属性：绿色字体 0000_0010b
	mov ds:[bx+1], ax	;更改高位
...
S_Stack2:
	...
	push ax			;将字符串入栈(一次2byte)
	sub bx, 2 	
	LOOP S_Stack2	;循环完，栈空间保存了改写后的字符串 
	;===================================================
	;屏幕第13行中间显示 绿底红字 的'Welcome to zhq3211.'
	;===================================================
	mov ax, 0B800H
	mov ds, ax	;显存缓冲区范围 B800:0000h~B800:7FFFh

	mov bx, 780h+3Ch 	;对应第13行(780h-81Fh),第30(3Ch=60)个字符
	
	mov cx, 19			;原字符串为19byte. 'Welcome to zhq3211.'
S2:	pop ax
	mov ds:[bx], ax		;出栈(低位为所求)
	
	mov ax, 00100100b	;字符属性: 绿底红字 0010_0100b=24h
	mov ds:[bx+1], ax	;更改高位
...
S_Stack3:
	...
	push ax			;将字符串入栈(一次2byte)
	sub bx, 2 	
	LOOP S_Stack3	;循环完，栈空间保存了改写后的字符串 
	;=========================================================
	;屏幕第15行中间显示 (闪烁)白底蓝色 的'Welcome to zhq3211.'
	;=========================================================
	mov ax, 0B800H
	mov ds, ax	;显存缓冲区范围 B800:0000h~B800:7FFFh

	mov bx, 8C0h+3Ch 	;对应第15行(8C0h-95Fh),第30(3Ch=60)个字符
	
	mov cx, 19			;原字符串为19byte. 'Welcome to zhq3211.'
S3:	pop ax
	mov ds:[bx], ax		;出栈(低位为所求)
	
	mov ax, 11110001b	;字符属性: (闪烁)白底蓝色 11110001b=71h
	mov ds:[bx+1], ax	;更改高位
